...
for k=1:N_iterations
    ...
    best(k)=min(F_best);                   % Compute best,worst and average objective function values
    average(k)=mean(F_best);                      % Every Iteration
    worst(k)=max(F_best);
        
end

disp('The Minimizer Point is');
disp(g_best);                            % Plot the Minimum Point
disp('The Objective function value at the Minimum Point is');
disp(g_best_value);
